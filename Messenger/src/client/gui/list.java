	...
	private void event() {
		...
		tree.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				for (int i = 0; i < friend.getListname().size(); i++) {	//노드 클릭시
					String nodes = node.toString();			//노드의 내용이
					if (nodes.equals(friend.getNickname().get(i))) {		//닉네임 배열에 있는 값들과 비교하여 같은 값이 있다면
						snames.setText(friend.getListname().get(i));		//해당 주소값에 해당하는 이름과 아이피를 출력
...
//		listip.add(3, "192.168.0.4");
	...
	private void load() {
		...
		for (int i = 0; i < friend.getListname().size(); i++) {		//로그인시 친구 목록 불러오는 메소드
